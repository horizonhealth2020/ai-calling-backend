...
- Convoso — lead management, call disposition, and call log retrieval
  ...
  - Auth: `CONVOSO_AUTH_TOKEN` passed as `auth_token` query parameter (root service) or via `CONVOSO_AUTH_TOKEN` env var (ops-api)
  - Endpoints used:
...
- `OPENAI_API_KEY` — for OpenAI call audit AI
